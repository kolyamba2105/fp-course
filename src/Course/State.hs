...
instance Functor (State s) where
  ...
  (<$>) ab (State run) = State (\s -> let (a, state) = run s in (ab a, state))
...
instance Applicative (State s) where
  ...
  (<*>) (State runAB) (State runA) = State (\s -> let (ab, nextState) = runAB s; (a, state) = runA nextState in (ab a, state))
...
instance Monad (State s) where
  ...
  (=<<) asb (State run) = State (\s -> let (a, state) = run s; (State runB) = asb a in runB state)
...
findM pf (a :. as) = (\b -> if b then pure $ Full a else findM pf as) =<< pf a
...
firstRepeat as = eval (findM p as) S.empty
  where
    p a = (\s -> const (pure (a `S.member` s)) =<< put (S.insert a s)) =<< get
...
distinct as = eval (filtering p as) S.empty
  where
    p a = (\s -> pure (a `S.notMember` s) <* put (S.insert a s)) =<< get
